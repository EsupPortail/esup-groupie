...
/**********************************************************************************************************************************************************************************************************************************/
    ...
    */
    ...
    public function mygroupsAction(LdapFonctions $ldapfonctions) {
        ...
        $arData = $ldapfonctions->recherche($this->config_groups['groupadmin']."=".$dnUser, array($this->config_groups['cn'], $this->config_groups['desc'], $this->config_groups['groupfilter']), 1, $this->config_groups['cn']);
        ...
        if (true === $this->get('security.authorization_checker')->isGranted('ROLE_ADMIN')) {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                if (strlen($cn) > $this->config_groups['max_name_size']) {
                    ...
                    return $this->render('Group/group.html.twig', array('form' => $form->createView(), 'phrase_regex' => $this->config_groups['phrase_regex']));
                }

                // Test validité nom du groupe
                $pattern = $this->config_groups['name_regex'];
                // On cherche les caracteres autorises
                preg_match($pattern,$cn,$result);
                if(!empty($result)){
        ...
        if (true === $this->get('security.authorization_checker')->isGranted('ROLE_ADMIN')) {
            ...
            if (true === $this->get('security.authorization_checker')->isGranted('ROLE_CREATEUR')) {
                ...
                $form = $this->createForm(GroupCreatorModifType::class,
                    array('action' => $this->generateUrl('group_modify', array('cn' => $cn, 'desc' => $desc, 'filt' => $filt, 'groupofgroup' => 'no')),
                        'method' => 'GET',
                        'liste_groupes' => $tab_choice_groups,
                        'prefixe' => $i_default,
                        'nom' => $nom,
